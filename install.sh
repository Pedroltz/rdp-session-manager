...
# Prevent apt from prompting for user input (timezone, keyboard, debconf, etc.)
...
case "$DISTRO_TYPE" in
    ...
    debian)
        ...
        ORACLE_DEPS=(libaio-dev unixodbc unixodbc-dev alien wget)
        ...
        # libaio1 foi renomeado para libaio1t64 no Debian 12+ / Ubuntu 24.04+
        # Tenta o nome novo primeiro, depois o antigo como fallback
        if is_pkg_installed_deb "libaio1t64"; then
            print_skip "libaio1t64"
        elif is_pkg_installed_deb "libaio1"; then
            print_skip "libaio1"
        else
            if sudo apt-get install -y libaio1t64 &>/dev/null; then
                print_success "libaio1t64 installed"
            elif sudo apt-get install -y libaio1 &>/dev/null; then
                print_success "libaio1 installed"
            else
                print_warning "libaio1/libaio1t64 not installed"
            fi
        fi
