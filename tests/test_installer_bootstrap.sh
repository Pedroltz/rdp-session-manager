...
PY
write_bundle_release
expect_failure "Could not extract the bundle." --dry-run --yes
cp "$TEST_DIR/assets/valid-bundle.zip" "$TEST_DIR/assets/$BUNDLE_NAME"
...
printf '%s\n' \
    ...
    > "$TEST_DIR/assets/release.json"
expect_failure "contains neither the bundle nor the required legacy assets" --dry-run --yes
...
expect_failure "SHA256SUMS does not contain the installer.pyz checksum." \
