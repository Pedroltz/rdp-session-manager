...
class MainWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, application, user_manager, rdp_config, de_installer, session_monitor, system_deps, **kwargs):
        ...
        # Create warning banner for xrdp
    ...
    def update_server_info(self):
        ...
        # Get active sessions count (only enabled users)
    ...
    def update_sessions_info(self):
        ...
        try:
            # Only count sessions from enabled users
            all_sessions = self.session_monitor.get_active_sessions()
            enabled_users = [u.username for u in self.user_manager.list_users() if u.enabled]

            # Filter enabled user sessions
            sessions_count = sum(1 for session in all_sessions if session.username in enabled_users)
    ...
    def create_user_row(self, user):
        ...
        # Different subtitle for RemoteApp vs Desktop
        ...
        # Status label - priority: 1) Disabled, 2) Connected, 3) Enabled
        if not user.enabled:
        ...
        status_label.set_margin_end(12) # Space before switch

        status_box.append(status_label)

        # Enable/Disable Switch (to the right of status)
        ...
        # Make the row clickable using GestureClick
        ...
        # Copy IP row (same style as above, without Button)
    ...
    def create_xrdp_warning_banner(self):
        ...
        self.xrdp_banner.set_title("WARNING xrdp server is not installed - Application will not work without it")
        ...
        if toolbar_view and isinstance(toolbar_view, Adw.ToolbarView):
            # Get the main box (toolbar content)
            content = toolbar_view.get_content()

            if content and isinstance(content, Gtk.Box):
                # Insert banner as the first child of the box
    ...
    def on_add_user(self, button):
        ...
        if not self.system_deps.is_xrdp_ready():
            # Show dialog stating that you need to install xrdp
            dialog = Adw.MessageDialog(
                transient_for=self,
                heading="xrdp is not installed",
                body="You need to install the xrdp server before creating RDP users.\n\nDo you want to install it now?"
            )
    ...
    def on_user_toggle(self, username, new_state, switch):
        ...
        # Block automatic switch change - let's control manually
        # Return True means "lock default change"
        ...
        def do_toggle():
            ...
            try:
                ...
                GLib.idle_add(self.show_toast, f"X Error changing status of {username}")
        ...
        # Return True to prevent automatic switching of the switch
        # (we will control manually after successful operation)
        return True
    ...
    def on_sudo_toggle(self, username, new_state, switch):
        ...
        if is_connected or has_processes:
            # Show warning about reconnection required
            action_text = "grant" if new_state else "revoke"
            dialog = Adw.MessageDialog(
                transient_for=self,
                heading=f"WARNING {username} is connected",
                body=f"""For {action_text} sudo privileges, the user session will be automatically terminated.

IMPORTANT NOTICE: Group changes only take effect after complete logout/login.

The user will need to reconnect via RDP for the administrator privileges to be {"applied" if new_state else "removed"}.

Do you want to continue?"""
            )
            dialog.add_response("cancel", "Cancel")
            dialog.add_response("continue", "Continue and Logout")
            ...
            # No active session, run directly
        ...
        def do_toggle():
            ...
            try:
                if new_state:
                    ...
                    if success:
                        GLib.idle_add(self.show_toast, f"OK Sudo privileges granted - Reconnect to apply")
                        # Update the switch manually
                        GLib.idle_add(switch.set_active, True)
                        # Update user list
                    ...
                    if success:
                        GLib.idle_add(self.show_toast, f"OK Sudo privileges revoked - Reconnect to apply")
                        # Update the switch manually
                        GLib.idle_add(switch.set_active, False)
                        # Update user list
                        GLib.timeout_add(500, self.load_users)
                ...
                GLib.idle_add(self.show_toast, f"X Error changing sudo privileges from {username}")
    ...
    def on_delete_user(self, username):
        ...
        if active_pids:
            # User has active session - show warning
            is_connected = self.session_monitor.is_user_connected(username)
            session_info = " is connected via RDP" if is_connected else f" has {len(active_pids)} active processes"

            dialog = Adw.MessageDialog(
                transient_for=self,
                heading=f"WARNING {username} is active",
                body=f"The user {username}{session_info}.\n\nTo remove the user, your sessions will be terminated automatically.\n\nDo you want to continue?"
            )
            ...
            # User has no active processes - normal confirmation
            dialog = Adw.MessageDialog(
                transient_for=self,
            ...
            try:
                ...
                if success:
                    ...
                    error_dialog = Adw.MessageDialog(
                        ...
                        body=f"Unable to remove user {username}. Make sure you have administrator permissions."
                    )
                ...
                error_dialog = Adw.MessageDialog(
                    transient_for=self,
                    heading="Error removing user",
                    body=f"Unable to remove user {username}.\n\nError: {str(e)}"
                )
                error_dialog.add_response("ok", "OK")
                error_dialog.present()
    ...
    def on_connect_rdp(self, user):
        ...
        if not self.system_deps.is_freerdp_installed():
            # Show dialog to install FreeRDP
    ...
    def on_user_settings(self, user, popover):
        ...
        # Create settings dialog using Adw.Dialog (not MessageDialog) to have full control of size
        dialog = Adw.Dialog()
        dialog.set_title(f"Edit RDP User")
        ...
        # Create main container with the same dimensions as the creation screen
        ...
        session_group.set_description("Choose between full desktop or single app")

        # Field: Session Type using AdwComboRow
        ...
        session_string_list.append("WineGE RemoteApp (Windows App)")
        ...
        # === Group: RemoteApp (Linux) ===
        remoteapp_group = Adw.PreferencesGroup()
        remoteapp_group.set_title("RemoteApp Linux")
        remoteapp_group.set_description("Set the Linux application to run")
        ...
        # Field: Arguments (Linux RemoteApp)
        ...
        def on_select_winege_exe(btn):
            ...
            # Hide the settings dialog temporarily
            ...
            def on_file_selected(source_object, result):
                ...
                try:
                    ...
                    if file:
                        ...
                        # If get_path() returns None, try URI
                        if not file_path:
                            import urllib.parse
                            uri = file.get_uri()
            ...
            # Open file dialog with MainWindow as parent
        ...
        # Button to list available executables
        ...
        def on_list_winege_exes(btn):
            ...
            # Search for available executables
            executables = self.user_manager.list_user_executables(user.username)

            if not executables:
                self.show_toast("X No executable found")
                return
            ...
            # Create listbox with executables
            scrolled = Gtk.ScrolledWindow()
        ...
        # Function to clean and normalize the path when the user types
        ...
        # Connect events: when pressing Enter or losing focus
        winege_exe_entry.connect('apply', lambda e: on_exe_path_changed(winege_exe_entry))

        # GTK4 uses controller for focus events
        ...
        # Function to toggle visibility based on session type
        ...
        security_group.set_description("Change RDP password (leave empty to keep the current one)")
        ...
        def on_save_clicked(button):
            ...
            if new_session_type == 'remoteapp':
                ...
                new_app_args = '' # WineGE does not use separate arguments

                # Validate that .exe is not empty and exists
                if not new_app_command:
            ...
            if new_password or confirm_password:
                ...
                if len(new_password) < 6:
                    self.show_toast("X Password must be at least 6 characters")
                    return
    ...
    def _apply_user_changes(self, dialog, user, new_username, new_fullname, new_session_type,
        ...
        def apply_changes():
            try:
                ...
                # 3. Change session type (if changed)
                if new_session_type != original_session_type:
                    ...
                    if new_session_type == 'remoteapp':
                        ...
                        # For desktop, use the user's current DE
                    ...
                    # Same type, but may have changed app/args
                    ...
                    # Same WineGE type, but may have changed exe/args
                    original_app = getattr(user, 'app_command', '')
                    original_args = getattr(user, 'app_args', '')

                    if new_app_command != original_app or new_app_args != original_args:
                        # Update executable if changed
                ...
                GLib.idle_add(self.show_toast, f"X Error updating settings")
    ...
    def show_password_dialog(self, user):
        ...
        dialog = Adw.MessageDialog(
            ...
            body=f"Enter credentials to connect via RDP."
        )
        ...
        # Use Entry with visibility=False instead of PasswordEntry
        password_entry = Gtk.Entry()
        password_entry.set_visibility(False)  # Hide text
        password_entry.set_invisible_char('•') # Character for password
        password_entry.set_hexpand(True)
        password_entry.set_can_focus(True)

        # Connect Enter in both fields to activate default button
    ...
    def on_password_dialog_response(self, dialog, response, user):
        ...
        if response == "connect":
            ...
            # Close dialog before opening FreeRDP
            dialog.close()
    ...
    def launch_freerdp_client(self, user, password=None, domain=None):
        ...
        try:
            ...
            # Check if it is RemoteApp to adjust parameters
            ...
            # For RemoteApp and Desktop, always use dynamic-resolution
            ...
            if process.poll() is not None:
                ...
                raise Exception(f"FreeRDP ended with code {process.returncode}: {error_msg[:200]}")
