...
class UserDialog(Adw.Dialog):
    ...
    def on_create_user(self, button):
        ...
        logger.info("=" * 60)
        logger.info("STARTING RDP USER CREATION")
        logger.info("=" * 60)
        ...
        if session_type in ['remoteapp', 'winege-remoteapp']:
            logger.info(f"→ Creating {session_type} user directly")
            self.create_user(username, password, fullname, de_id, session_type, app_command, app_args)
        elif install_de and not de_installed:
            logger.info(f"→ Desktop environment '{de_id}' must be installed first")
            self.install_de_and_create_user(de_id, username, password, fullname, session_type, app_command, app_args)
        else:
            if not de_installed:
                logger.warning(f"WARNING DE '{de_id}' is not installed, but the user chose not to install it")
            logger.info(f"→ Creating user directly (install DE: {install_de})")
            self.create_user(username, password, fullname, de_id, session_type, app_command, app_args)
    ...
    def install_de_and_create_user(self, de_id, username, password, fullname,
        ...
        progress_dialog = Adw.MessageDialog(
            ...
            body="Installing the desktop environment...\nThis may take several minutes."
        )
    ...
    def create_user(self, username, password, fullname, de_id,
        ...
        def on_create_complete(success, user_or_error):
            ...
            if success:
                ...
                error = user_or_error
                logger.error(f"X ERROR CREATING USER {username}: {error}")
        ...
        def create_in_thread():
            try:
                ...
                GLib.idle_add(append_log, f"Username: {username}")

                if session_type == 'desktop':
                    GLib.idle_add(append_log, "Type: Full Desktop")
                ...
                GLib.idle_add(append_log, "→ Checking the rdp-users group...")
                ...
                if user:
                    ...
                    GLib.idle_add(append_log, "  Setup complete!")
...
Windows:
  ...
  Enter: {self.get_connection_info(user.rdp_port)}

The user is already shown in the main list!"""
        )
    ...
    def show_error_dialog(self, error):
        ...
        error_dialog = Adw.MessageDialog(
            ...
            body=f"The user could not be created.\n\nError: {error}\n\nCheck that:\n• You have administrator permissions\n• The username does not already exist\n• The rdp-users group was created"
        )
        error_dialog.add_response("ok", "OK")
        error_dialog.present()
