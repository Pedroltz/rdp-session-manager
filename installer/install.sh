...
ensure_python() {
    ...
    for arg in "$@"; do
        if [[ "$arg" == "--dry-run" ]]; then
            fail "--dry-run needs Python 3 already installed; no packages have been changed."
        fi
    done
    log "Python 3 not found; installing only the installer runtime."
    [ -r /etc/os-release ] || fail "Unable to detect distribution to install Python."
    ...
    if [[ "$id_like" == *debian* || "$id_like" == *ubuntu* ]]; then
        ...
        fail "Distribution not supported and Python 3 is not installed."
    fi
    command -v python3 >/dev/null 2>&1 || fail "Unable to install Python 3."
}
...
try:
    ...
    raise SystemExit(f"Unable to interpret release response: {exc}")
...
if (
    ...
    raise SystemExit("The selected release is invalid or has not yet been published.")
if sys.argv[3] == "true" and release.get("prerelease"):
    raise SystemExit("No stable releases were found on GitHub.")

assets = release.get("assets")
if not isinstance(assets, list):
    raise SystemExit("The release does not contain a valid list of assets.")
...
if bundle is not None:
    ...
    if missing:
        raise SystemExit(
            "The release does not contain the bundle or required legacy assets: "
            + ", ".join(missing)
        )
    ...
    if not all(isinstance(url, str) and url for url in urls):
        raise SystemExit("The release's legacy assets do not have valid URLs.")
    print("legacy")
    print(*urls, sep="\n")
PY
)
[ "${#release_fields[@]}" -ge 4 ] || fail "Unable to resolve release assets."
...
if [ "$release_mode" = "bundle" ]; then
    ...
    log "Downloading bundle from release ${release_tag}..."
    ...
    mkdir -p "$bundle_dir"
    python3 - "$bundle_path" "$bundle_dir" <<'PY' || fail "Unable to extract bundle."
import stat
import sys
import zipfile
...
with zipfile.ZipFile(archive_path) as archive:
    ...
    if names != expected:
        ...
        raise SystemExit(f"Unexpected content in bundle; missing={missing}, extras={extra}")
...
PY
    (cd "$bundle_dir" && sha256sum --check --strict SHA256SUMS >/dev/null) \
        || fail "Invalid internal checksum in bundle."
    ...
    log "Downloading legacy visual installer for release ${release_tag}..."
    ...
    [ -n "$expected" ] || fail "SHA256SUMS does not contain the checksum of installer.pyz."
...
fail "Interactive installation requires a terminal. Run in a terminal or use --yes."
